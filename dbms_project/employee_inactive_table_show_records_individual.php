...
$sql = "SELECT* FROM employee_inactive WHERE empid=$empid";
...
if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
     ...
     "    [city]: ".$row["city"]. "    [state]: ".$row["state"]. "    [date_of_joining]: ".$row["date_of_joining"]. "    [date_of_leaving]: ".$row["date_of_leaving"]."    [email]: ".$row["email"]."<br>";
   }
} else {
   echo "0 results";
}
